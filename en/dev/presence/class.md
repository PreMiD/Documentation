...
You can find the documentation for the `Slideshow` class
[here](/dev/presence/slideshow).
...
let strings: LangStrings = await getStrings(),
  // The ID is the ID of the multiLanguage setting.
  oldLang = await presence.getSetting("ID").catch(() => "en");
...
const playString = strings.play, // result: Playing
  pauseString = strings.pause; // result: Paused
```
...
The `PresenceData` interface is recommended to use when you are using the
`setActivity()` method.
