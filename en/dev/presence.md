...
presence.on("UpdateData", async () => {
  ...
  if (presenceData.details) presence.setActivity(presenceData); /*Update the presence with no data, therefore clearing it and making the large image the Discord Application icon, and the text the Discord Application name*/
  else presence.setActivity(); //Update the presence with all the values from the presenceData object
});
```
...
- If that doesn't help you either then ask a presence developer on our
  [Discord server](https://discord.premid.app/) for help.
