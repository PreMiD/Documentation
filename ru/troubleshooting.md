...
**Настройки пользователя** > **Статус активности** ![activitystatus.png](/activitystatus.png)

### Убедитесь, что Discord запущен НЕ от имени администратора
Очень важно. Discord RPC не будет работать, если вы запустили Discord от имени администратора.

### Вы используете presence с настройками?
Многие presence (в том числе `Twitch` и `SoundCloud`) затронуты проблемой расширения. Эта проблема приводит к тому, что расширение не получает значения настроек по умолчанию должным образом.
